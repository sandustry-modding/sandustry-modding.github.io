# Energy

Power networks, collector behavior, and signal targets.

## Concepts

- [Networks and signals](/okf/energy/networks-and-signals.md)
- [API: energy](/okf/energy/api-energy.md)
- [API: signals](/okf/energy/api-signals.md)
- [API: resources (energy)](/okf/energy/api-resources.md)
- [Shared state](/okf/energy/shared-state.md)
- [Engine energy](/okf/energy/engine-energy.md)
- [Engine signals runtime](/okf/energy/engine-signals.md)
- [Clipboard signal links](/okf/energy/clipboard.md)
- [Gold, collector, and power](/okf/energy/gold-collector.md)
- [Gaps](/okf/energy/gaps.md)
- [Probe](/okf/energy/probe.md)
